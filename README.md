...
自分の X (Twitter) の投稿を一括削除するための、**ローカル専用**の Web アプリです。
`npm run dev` で API サーバー (`127.0.0.1:5174`) と UI (`127.0.0.1:5173`) が立ち上がり、ブラウザから条件を指定して削除します。
認証情報も削除ログもこのマシンの `data/` に置かれるだけで、外部に出る通信は x.com 宛てのものだけです。

---

## ⚠️ 先に読んでください

> **削除は取り消せません。** X 側に復元手段はありません。twedel が残せるのは、削除前に `data/deleted-log.ndjson` へ書き出した本文のコピーだけです。
>
