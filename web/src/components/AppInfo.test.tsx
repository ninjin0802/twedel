...
describe('AppInfo', () => {
  it('shows the running version on the about page', () => {
    const html = renderToStaticMarkup(<AppInfo page="about" version="0.9.0" />);
    expect(html).toContain('Version 0.9.0');
  ...
  it('shows release notes newest first', () => {
    ...
    expect(RELEASES[0].version).toBe('0.9.0');
    expect(RELEASES[0].changes).toContain('取得結果を受信後すぐジョブメモリから解放');
    expect(html.indexOf('v0.9.0')).toBeLessThan(html.indexOf('v0.8.1'));
  ...
  it('shows an accessible progress bar while downloading an update', () => {
    const html = renderToStaticMarkup(<AppInfo page="about" version="0.9.0" updateState={{ status: 'downloading', percent: 42 }} />);
