...
describe('AppInfo', () => {
  it('shows the running version on the about page', () => {
    const html = renderToStaticMarkup(<AppInfo page="about" version="0.4.3" />);
    expect(html).toContain('Version 0.4.3');
  ...
  it('shows release notes newest first', () => {
    ...
    expect(RELEASES[0].version).toBe('0.4.3');
    expect(RELEASES[0].changes).toContain('更新のダウンロード完了後にインストーラーをバックグラウンドで自動実行');
    expect(html.indexOf('v0.4.3')).toBeLessThan(html.indexOf('v0.4.2'));
