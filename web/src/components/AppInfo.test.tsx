...
describe('AppInfo', () => {
  it('shows the running version on the about page', () => {
    const html = renderToStaticMarkup(<AppInfo page="about" version="0.5.0" />);
  ...
  it('shows release notes newest first', () => {
    ...
    expect(RELEASES[0].version).toBe('0.5.0');
    expect(RELEASES[0].changes).toContain('デスクトップ向け左サイドバーとモバイル向けドロワーを追加');
    expect(html.indexOf('v0.5.0')).toBeLessThan(html.indexOf('v0.4.9'));
  ...
  it('shows an accessible progress bar while downloading an update', () => {
    const html = renderToStaticMarkup(<AppInfo page="about" version="0.5.0" updateState={{ status: 'downloading', percent: 42 }} />);
