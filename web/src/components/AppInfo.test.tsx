...
describe('AppInfo', () => {
  it('shows the running version on the about page', () => {
    const html = renderToStaticMarkup(<AppInfo page="about" version="0.4.9" />);
    expect(html).toContain('Version 0.4.9');
  ...
  it('shows release notes newest first', () => {
    ...
    expect(RELEASES[0].version).toBe('0.4.9');
    expect(RELEASES[0].changes).toContain('READMEを一般利用者向けに分かりやすく再構成');
    expect(html.indexOf('v0.4.9')).toBeLessThan(html.indexOf('v0.4.8'));
  ...
  it('shows an accessible progress bar while downloading an update', () => {
    const html = renderToStaticMarkup(<AppInfo page="about" version="0.4.9" updateState={{ status: 'downloading', percent: 42 }} />);
