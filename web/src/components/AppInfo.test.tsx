...
describe('AppInfo', () => {
  it('shows the running version on the about page', () => {
    const html = renderToStaticMarkup(<AppInfo page="about" version="0.4.2" />);
    expect(html).toContain('Version 0.4.2');
  ...
  it('shows release notes newest first', () => {
    ...
    expect(RELEASES[0].version).toBe('0.4.2');
    expect(RELEASES[0].changes).toContain('アプリアイコンを新しいデザインへ刷新');
    expect(html.indexOf('v0.4.2')).toBeLessThan(html.indexOf('v0.4.1'));
