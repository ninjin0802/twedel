...
describe('AppInfo', () => {
  it('shows the running version on the about page', () => {
    const html = renderToStaticMarkup(<AppInfo page="about" version="0.11.0" />);
    expect(html).toContain('Version 0.11.0');
  ...
  it('shows release notes newest first', () => {
    ...
    expect(RELEASES[0].version).toBe('0.11.0');
    expect(RELEASES[0].changes).toContain('コード署名ポリシーとプライバシーポリシーを公開');
    expect(html.indexOf('v0.11.0')).toBeLessThan(html.indexOf('v0.10.0'));
  ...
  it('shows an accessible progress bar while downloading an update', () => {
    const html = renderToStaticMarkup(<AppInfo page="about" version="0.11.0" updateState={{ status: 'downloading', percent: 42 }} />);
