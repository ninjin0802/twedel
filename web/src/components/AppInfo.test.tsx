...
describe('AppInfo', () => {
  it('shows the running version on the about page', () => {
    const html = renderToStaticMarkup(<AppInfo page="about" version="0.4.8" />);
    expect(html).toContain('Version 0.4.8');
  ...
  it('shows release notes newest first', () => {
    ...
    expect(RELEASES[0].version).toBe('0.4.8');
    expect(RELEASES[0].changes).toContain('新しいバージョンの通知にGitHub Releaseの更新内容を表示');
    expect(html.indexOf('v0.4.8')).toBeLessThan(html.indexOf('v0.4.7'));
  ...
  it('shows an accessible progress bar while downloading an update', () => {
    const html = renderToStaticMarkup(<AppInfo page="about" version="0.4.8" updateState={{ status: 'downloading', percent: 42 }} />);
