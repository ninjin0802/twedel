...
describe('AppInfo', () => {
  it('shows the running version on the about page', () => {
    const html = renderToStaticMarkup(<AppInfo page="about" version="0.7.0" />);
  ...
  it('shows release notes newest first', () => {
    ...
    expect(RELEASES[0].changes).toContain('複数のXアカウントを端末内へ安全に保存');
    expect(html.indexOf('v0.7.0')).toBeLessThan(html.indexOf('v0.6.0'));
  ...
  it('shows an accessible progress bar while downloading an update', () => {
    const html = renderToStaticMarkup(<AppInfo page="about" version="0.7.0" updateState={{ status: 'downloading', percent: 42 }} />);
