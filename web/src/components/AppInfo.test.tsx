...
describe('AppInfo', () => {
  it('shows the running version on the about page', () => {
    const html = renderToStaticMarkup(<AppInfo page="about" version="0.7.2" />);
  ...
  it('shows release notes newest first', () => {
    ...
    expect(RELEASES[0].version).toBe('0.7.2');
    expect(RELEASES[0].changes).toContain('データのないカーソルを無制限にたどる問題を修正');
    expect(html.indexOf('v0.7.2')).toBeLessThan(html.indexOf('v0.7.1'));
  ...
  it('shows an accessible progress bar while downloading an update', () => {
    const html = renderToStaticMarkup(<AppInfo page="about" version="0.7.2" updateState={{ status: 'downloading', percent: 42 }} />);
