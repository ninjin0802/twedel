...
describe('AppInfo', () => {
  it('shows the running version on the about page', () => {
    const html = renderToStaticMarkup(<AppInfo page="about" version="0.4.7" />);
    expect(html).toContain('Version 0.4.7');
  ...
  it('shows release notes newest first', () => {
    ...
    expect(RELEASES[0].changes).toContain('GitHub Release本文の改行表示を修正');
    expect(html.indexOf('v0.4.7')).toBeLessThan(html.indexOf('v0.4.6'));
  ...
  it('shows an accessible progress bar while downloading an update', () => {
    const html = renderToStaticMarkup(<AppInfo page="about" version="0.4.7" updateState={{ status: 'downloading', percent: 42 }} />);
