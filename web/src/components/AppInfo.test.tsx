...
describe('AppInfo', () => {
  it('shows the running version on the about page', () => {
    const html = renderToStaticMarkup(<AppInfo page="about" version="0.7.1" />);
  ...
  it('shows release notes newest first', () => {
    ...
    expect(RELEASES[0].changes).toContain('重複や埋め込みだけの中間ページを越えて取得を継続');
    expect(html.indexOf('v0.7.1')).toBeLessThan(html.indexOf('v0.7.0'));
  ...
  it('shows an accessible progress bar while downloading an update', () => {
    const html = renderToStaticMarkup(<AppInfo page="about" version="0.7.1" updateState={{ status: 'downloading', percent: 42 }} />);
