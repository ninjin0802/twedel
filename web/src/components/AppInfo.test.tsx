...
describe('AppInfo', () => {
  it('shows the running version on the about page', () => {
    const html = renderToStaticMarkup(<AppInfo page="about" version="0.5.1" />);
  ...
  it('shows release notes newest first', () => {
    ...
    expect(RELEASES[0].version).toBe('0.5.1');
    expect(RELEASES[0].changes).toContain('絞り込み画面の「リポスト」を初期状態でチェック済みに変更');
    expect(html.indexOf('v0.5.1')).toBeLessThan(html.indexOf('v0.5.0'));
  ...
  it('shows an accessible progress bar while downloading an update', () => {
    const html = renderToStaticMarkup(<AppInfo page="about" version="0.5.1" updateState={{ status: 'downloading', percent: 42 }} />);
