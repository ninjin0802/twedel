...
describe('AppInfo', () => {
  it('shows the running version on the about page', () => {
    const html = renderToStaticMarkup(<AppInfo page="about" version="0.12.0" />);
    expect(html).toContain('Version 0.12.0');
    ...
    expect(html).toContain('X: @_nin82');
    expect(html).toContain('Xプロフィールを標準ブラウザで開く');
  ...
  it('shows an accessible progress bar while downloading an update', () => {
    const html = renderToStaticMarkup(<AppInfo page="about" version="0.12.0" updateState={{ status: 'downloading', percent: 42 }} />);
